...
Converting the [expo example](https://codesandbox.io/s/github/expo/examples/tree/master/with-react-router) with `react-router` to `tyler`


- add an entry file called `index.js` with code like below
```


```
- comment/remove `export default App;` from end of `App.js`
- remove and `react-router` dependencies
- add `deepmerge`, `typescript`, `react-native-easy-grid` as additional dependency. 
- add `import merge from deepmerge` in your entry file (refer to example)
- import all components in `index.js` and create an object called componentsSet that you will pass to <GridSection> as a property by same name
- Important point add a `label` property to each compoinent that will become it's identifier for becoming a render area. This shall be unique
- MOST important :: create a layout config - `layout.js` for your desired application like below
```
